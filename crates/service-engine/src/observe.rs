...
pub fn record_outbox_depth(depth: i64, oldest_age_seconds: Option<f64>) {
    metrics::gauge!(OUTBOX_PENDING, identity()).set(depth.max(0) as f64);
    metrics::gauge!(OUTBOX_OLDEST_AGE_SECONDS, identity()).set(oldest_age_seconds.unwrap_or(0.0));
}
