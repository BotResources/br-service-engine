...
#[tokio::test]
async fn s005_a_connection_cancelled_after_it_went_live_is_reaped_with_its_stream() {
...
#[tokio::test]
async fn s005_a_burst_beyond_the_held_bound_opens_the_session_on_the_truth_the_database_holds() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
