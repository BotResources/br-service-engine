...
#[tokio::test]
async fn s080_a_nats_outage_past_its_grace_window_takes_the_pod_out_of_rotation_with_the_reason() {
    let db = TestDb::fresh().await;
    let mut nats = TestNats::spawn().await;
    let fabric = nats.nats().await;
    let pool = db.app_pool().clone();

    let config = engine_config("se_s080_nats_grace", "pod-nats-grace").with_nats_grace(NATS_GRACE);
