...
async fn s179_a_row_the_reachable_broker_keeps_rejecting_is_dead_lettered_not_abandoned() {
    ...
    let relay = OutboxRelay::new(pool.clone(), fabric).with_dead_letters(DeadLetters::new(pool.clone()));
    ...
    for attempt in 1..MAX_ATTEMPTS {
        ...
        assert_eq!(pass.dead_lettered, 0, "attempt {attempt}: still under the bound");
        assert_eq!(
    ...
    assert_eq!(source, "outbox", "surfaced under the outbox source, not silently gone");
    assert_eq!(message_id, staged, "the dead letter carries the row's own message id");
