...
#[tokio::test]
async fn s043_row_claim_never_drains_the_same_row_concurrently() {
...
#[tokio::test]
async fn s043_a_row_staged_with_an_impact_is_published_within_one_window() {
    let db = TestDb::fresh().await;
...
#[tokio::test]
async fn s043_a_crash_between_publish_and_status_transition_republishes_once() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
...
#[tokio::test]
async fn s043_a_failing_relay_backs_off_without_delaying_the_others() {
    let db = TestDb::fresh().await;
