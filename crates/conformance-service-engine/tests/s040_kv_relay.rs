...
#[tokio::test]
async fn s040_the_kv_relay_publishes_monotonically_under_leadership() {
...
#[tokio::test]
async fn s040_a_change_requeued_mid_drain_is_never_marked_applied_unpublished() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
...
#[tokio::test]
async fn s040_a_stale_put_after_a_newer_retract_never_resurrects_the_key() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
