...
#[tokio::test]
async fn s044_the_beat_collects_the_slot_rows_of_a_job_that_already_ran() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let config = engine_config("se_s044_gc", "se-cron-0").with_beat(BEAT);
...
#[tokio::test]
async fn s044_a_slot_whose_row_the_collector_already_took_is_never_run_a_second_time() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let config = engine_config("se_s044_horizon", "se-cron-0").with_beat(BEAT);
...
#[tokio::test]
async fn s044_a_beat_whose_relays_keep_asking_for_another_round_still_stops_when_it_is_told_to() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let config = engine_config("se_s044_busy", "se-beat-0")
