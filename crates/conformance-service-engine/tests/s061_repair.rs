...
#[tokio::test]
async fn s061_a_principal_refresh_failure_ends_the_session_fail_closed() {
...
#[tokio::test]
async fn s061_a_failed_repair_never_resets_from_stale_state_and_ends_after_retries() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
