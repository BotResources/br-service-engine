...
const CHANNEL: &str = "se_s010_engine";
const WINDOW: Duration = Duration::from_millis(800);

#[tokio::test]
async fn s010_engine_a_lone_impact_after_idle_is_delivered_before_the_window_elapses() {
    ...
    let engine = spy_engine(
        &db,
        fabric,
        engine_config(CHANNEL, "pod-s010").with_window(WINDOW),
