...
#[tokio::test]
async fn s017_a_window_the_service_asked_to_run_under_rls_is_refused_without_an_applier() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
...
#[tokio::test]
async fn s017_a_window_attached_without_a_principal_resolver_is_refused() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
