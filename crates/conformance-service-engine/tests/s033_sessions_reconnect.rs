...
const LISTENER: &str = "se_s033_sessions_listener";
const QUERY_CHANNEL: &str = "se_s033_query_impact";
const QUERY_LISTENER: &str = "se_s033_query_listener";
...
#[tokio::test]
async fn s033_a_transport_reconnect_resets_every_session_and_resumes() {
...
#[tokio::test]
async fn s033_a_query_window_keeps_what_it_discovered_across_a_reconnect() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
