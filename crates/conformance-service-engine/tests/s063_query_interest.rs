...
#[tokio::test]
async fn s063_a_query_window_is_re_evaluated_by_a_noun_its_interest_declares_but_its_projector_does_not_render()
 {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
...
#[tokio::test]
async fn s063_query_membership_grows_on_a_foreign_and_on_a_principal_impact() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
...
#[tokio::test]
async fn s063_a_key_the_predicate_discovered_is_dropped_when_the_re_evaluation_disowns_it() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
