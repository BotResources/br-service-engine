...
use conformance_service_engine::inbound_support::{
    claim_rows, dead_letter_rows, effect_present, effect_writer, publish, sample_subscription,
    start_loop,
};
...
#[tokio::test]
async fn s29_a_crash_before_commit_redelivers_and_the_effect_lands_exactly_once() {
    ...
    assert_eq!(
        effect_present(db.app_pool(), id).await,
        1,
