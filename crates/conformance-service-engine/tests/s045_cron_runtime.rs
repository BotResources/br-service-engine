...
#[tokio::test]
async fn s045_a_five_field_job_and_an_interval_job_each_run_once_per_slot_across_two_skewed_runtimes()
 {
...
#[tokio::test]
async fn s045_a_slot_missed_while_no_runtime_was_up_runs_once_on_the_first_restart() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
...
#[tokio::test]
async fn s045_a_job_killed_mid_run_is_re_run_once_on_the_other_runtime_after_the_lease_expires() {
    let db = TestDb::fresh().await;
...
#[tokio::test]
async fn s045_a_job_that_outlives_a_beat_renews_its_lease_and_holds_no_transaction_open() {
    let db = TestDb::fresh().await;
    let pool = pool_named(&db, db.app_role(), "se_s045_long").await;
