...
#[tokio::test]
async fn s101_a_saved_noun_is_offered_by_the_leader_after_the_commit() {
...
#[tokio::test]
async fn s101_a_rolled_back_mutation_leaves_no_dirty_key_and_nothing_offered() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let pool = db.app_pool().clone();
...
#[tokio::test]
async fn s101_a_noun_that_stops_being_offerable_is_retracted() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let pool = db.app_pool().clone();
...
#[tokio::test]
async fn s101_boot_reconcile_repairs_a_bucket_that_drifted() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let pool = db.app_pool().clone();
