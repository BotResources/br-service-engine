...
#[tokio::test]
async fn s051_a_session_survives_no_longer_than_the_engine_when_a_worker_stops() {
    ...
    let mut engine = Engine::boot(
        engine_config("se_s051_worker_stop", "pod-worker-stop"),
        pool.clone(),
        fabric,
        readiness.clone(),
    )
...
#[tokio::test]
async fn s051_a_session_ends_when_the_render_worker_panics_the_one_path_a_sibling_await_cannot_close()
 {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    let fabric = nats.nats().await;
    ...
    let config = engine_config("se_s051_render_panic", "pod-render-panic");
    let readiness = ReadinessHandle::ready();
