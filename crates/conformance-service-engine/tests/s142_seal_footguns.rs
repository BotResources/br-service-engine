...
#[tokio::test]
async fn s142_sealing_an_already_sealed_key_is_refused_with_already_sealed() {
...
#[tokio::test]
async fn s142_a_chunk_beyond_the_declared_last_seq_fails_the_seal_and_is_never_dropped() {
    let db = TestDb::fresh().await;
    let runtime = note_body_runtime(db.app_pool().clone(), StagingTransport::silent(), RETENTION);
    let key = key();
    for chunk in SyntheticSource::new(&runtime, key.clone())
...
#[tokio::test]
async fn s142_a_slow_seal_does_not_block_another_keys_flush() {
    let db = TestDb::fresh().await;
    let runtime = note_body_runtime(db.app_pool().clone(), StagingTransport::silent(), RETENTION);
    let held = key();
    let other = key();
...
#[tokio::test]
async fn s142_a_registered_accumulator_without_a_service_fails_loud_at_boot() {
    let db = TestDb::fresh().await;
    ...
    let mut engine = Engine::<SamplePrincipal>::boot(
        engine_config("se_s142_serviceless", "pod-s142"),
        db.app_pool().clone(),
        fabric,
