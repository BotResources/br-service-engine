...
const LISTENER: &str = "se_s034_listener";
const GARBLED_CHANNEL: &str = "se_s034_garbled_impact";
const FROM_A_NEWER_POD: &str = r#"{"g":"019f8137-e784-7320-87f3-13074aacc4d4","p":0,"n":1,"i":[{"kind":"a_shape_a_newer_pod_stages"}]}"#;

#[tokio::test]
async fn s034_transport_reconnect() {
    let db = TestDb::fresh().await;
    let pool = pool_named(&db, db.app_role(), LISTENER).await;
    let transport = PgListenNotify::connect(pool, &engine_config("se_s034_impact", "pod-a"))
        .await
...
#[tokio::test]
async fn s034_a_payload_this_build_cannot_parse_is_reported_and_repaired_never_silently_fatal() {
    let db = TestDb::fresh().await;
    let pool = pool_named(&db, db.app_role(), "se_s034_garbled").await;
