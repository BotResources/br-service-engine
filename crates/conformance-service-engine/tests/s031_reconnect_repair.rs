...
const REPAIR_LISTENER: &str = "se_s031_repair_listener";

#[tokio::test]
async fn s031_the_beat_alone_repairs_a_pending_session_with_no_impact_after_the_outage() {
    ...
    let config = engine_config("se_s031_repair", "pod-repair")
...
#[tokio::test]
async fn s031_the_beat_ends_a_session_whose_repair_never_succeeds() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
    ...
    let spy = Spy::new();
    let switch = Arc::new(AtomicBool::new(false));
    let config = engine_config("se_s031_doomed", "pod-doomed")
