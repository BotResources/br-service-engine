...
const COALESCED: &str = "se_s013_coalesced";
const PER_IMPACT: &str = "se_s013_per_impact";

#[tokio::test]
async fn s013_engine_three_impacts_on_one_key_in_one_window_are_one_coalesced_upsert() {
...
#[tokio::test]
async fn s013_engine_per_impact_emits_one_delta_per_cause() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
