...
#[tokio::test]
async fn s023_a_membership_change_beyond_the_reset_threshold_is_a_reset() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
...
#[tokio::test]
async fn s023_a_head_member_that_vanishes_alone_is_removed_and_forgotten() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
