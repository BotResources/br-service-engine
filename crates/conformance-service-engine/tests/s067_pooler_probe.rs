...
async fn s067_pooler_probe() {
    ...
    let refused = Engine::<SamplePrincipal>::boot_with_probe(
        engine_config("se_s067_deaf_impact", "pod-a").with_listener_probe_timeout(PROBE_TIMEOUT),
        pool_named(&db, db.app_role(), "se_s067_deaf").await,
        fabric.clone(),
    ...
    let engine = Engine::<SamplePrincipal>::boot(
        engine_config("se_s067_healthy_impact", "pod-b").with_listener_probe_timeout(PROBE_TIMEOUT),
        pool_named(&db, db.app_role(), "se_s067_healthy").await,
        fabric,
