...
#[tokio::test]
async fn s058_a_projector_whose_key_does_not_decode_the_noun_key_is_refused_at_registration() {
    let mut registry = registry();
...
#[tokio::test]
async fn s058_a_projector_rendering_a_noun_no_type_declares_is_refused_at_registration() {
...
#[tokio::test]
async fn s058_the_engine_checks_projector_keys_against_the_bound_noun_not_the_first_registrant() {
    let db = TestDb::fresh().await;
    ...
    let mut engine = Engine::<SamplePrincipal>::boot(
        engine_config("se_s058_noun_bind", "pod-s058"),
        db.app_pool().clone(),
        fabric,
