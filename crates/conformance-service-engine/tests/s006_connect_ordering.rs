...
#[tokio::test]
async fn s006_an_impact_committed_during_the_snapshot_is_neither_lost_nor_duplicated() {
...
#[tokio::test]
async fn s006_an_impact_the_snapshot_already_saw_yields_no_second_delta() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
...
#[tokio::test]
async fn s006_a_dropped_stream_reaps_its_session_on_the_next_pass() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
    let home = Uuid::now_v7();
    let principal = member(&pool, Uuid::now_v7(), home).await;
