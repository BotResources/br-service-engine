...
const CHANNEL: &str = "se_s132_streaming_boot";

#[tokio::test]
async fn s132_a_registered_accumulator_without_its_streaming_stream_keeps_the_pod_down() {
    ...
    let mut engine = Engine::<SamplePrincipal>::boot(
        engine_config(CHANNEL, "pod-s132-absent").with_service("s132absent"),
        db.app_pool().clone(),
        fabric,
    ...
    assert!(
        matches!(outcome, Err(EngineError::Nats(NatsError::NoStream { ref name })) if name == "STREAMING_s132absent"),
...
#[tokio::test]
async fn s132_a_seal_retention_below_the_stream_max_age_fails_the_boot_loud() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    nats.provision_streaming("s132mismatch", Duration::from_secs(600))
        .await;
    let fabric = nats.nats().await;

    let mut engine = Engine::<SamplePrincipal>::boot(
        engine_config(CHANNEL, "pod-s132-mismatch")
            .with_service("s132mismatch")
    ...
    assert!(
        matches!(
            ...
            Err(EngineError::SealRetentionTooShort { ref stream, seal_retention, max_age })
                if stream == "STREAMING_s132mismatch"
