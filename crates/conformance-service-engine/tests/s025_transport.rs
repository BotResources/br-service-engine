...
#[tokio::test]
async fn s025_the_staging_pod_hears_itself() {
    let db = TestDb::fresh().await;
    let pool = pool_named(&db, db.app_role(), "se_s025_self").await;
    let transport = PgListenNotify::connect(pool, &engine_config("se_s025_self", "pod-a"))
        .await
...
#[tokio::test]
async fn s025_a_list_above_the_notify_limit_is_split_and_fully_heard() {
    let db = TestDb::fresh().await;
    let pool = pool_named(&db, db.app_role(), "se_s025_split").await;
    let transport = PgListenNotify::connect(pool, &engine_config("se_s025_split", "pod-a"))
        .await
