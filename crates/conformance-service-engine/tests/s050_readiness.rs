...
#[tokio::test]
async fn s050_readiness_is_down_until_the_listener_is_established() {
    ...
    let booting = tokio::spawn({
        let pool = pool.clone();
        let config = engine_config("se_s050_impact", "pod-a");
...
#[tokio::test]
async fn s050_a_refused_boot_leaves_readiness_down_with_the_operator_reason() {
    let db = TestDb::fresh().await;
    let readiness = ReadinessHandle::ready();

    let refused = establish_transport(
        db.superuser_pool().await,
        &engine_config("se_s050_refused", "pod-a"),
        &readiness,
    )
    .await
