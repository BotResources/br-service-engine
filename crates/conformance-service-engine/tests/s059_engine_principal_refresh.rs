...
#[tokio::test]
async fn s059_engine_revoking_a_local_fact_drops_the_row_on_the_same_pass() {
...
#[tokio::test]
async fn s059_engine_a_resolver_returning_none_ends_every_session_of_that_principal() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
    ...
    let engine = spy_engine(
        &db,
        fabric,
        engine_config("se_s059_revoked", "pod-s22b"),
        SpyAssignments::new(Spy::new()),
    )
    .await;
    let readiness = engine.readiness();
