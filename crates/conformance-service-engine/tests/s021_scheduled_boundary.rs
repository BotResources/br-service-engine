...
#[tokio::test]
async fn s021_a_scheduled_boundary_is_claimed_staged_and_deleted_in_one_transaction() {
    let db = TestDb::fresh().await;
    let pool = pool_named(&db, db.app_role(), "se_s021_scheduled").await;
    let transport = PgListenNotify::connect(pool, &engine_config("se_s021_scheduled", "pod-a"))
        .await
...
#[tokio::test]
async fn s021_a_boundary_fires_once_on_the_beat_across_two_engines_on_one_database() {
    let db = TestDb::fresh().await;
    let config = engine_config("se_s021_beat", "pod-a");
    let one = Arc::new(
        PgListenNotify::connect(
            pool_named(&db, db.app_role(), "se_s021_beat_a").await,
            &config,
        )
        .await
    ...
    let two = Arc::new(
        PgListenNotify::connect(
            pool_named(&db, db.app_role(), "se_s021_beat_b").await,
            &engine_config("se_s021_beat", "pod-b"),
        )
        .await
    ...
    let mut second = Beat::from_config(&engine_config("se_s021_beat", "pod-b"))
        .expect("a beat from the engine config")
