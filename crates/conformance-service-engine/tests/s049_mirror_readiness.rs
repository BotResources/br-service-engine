...
#[tokio::test]
async fn s049_readiness_is_down_until_the_directory_mirror_converges_and_down_again_when_it_dies() {
    ...
    assert!(
        ...
        "a dead mirror takes the service out of rotation, and names its own failure after the \
         fixed operator copy"
    );
...
#[tokio::test]
async fn s049_a_fabric_relay_that_cannot_publish_takes_a_converged_service_out_of_rotation() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    let fabric = nats.nats().await;
