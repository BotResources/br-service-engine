...
#[tokio::test]
async fn s046_a_cron_slot_is_named_by_its_fire_time_and_runs_once_across_two_skewed_pods() {
...
#[tokio::test]
async fn s046_a_missed_slot_is_claimable_after_the_fact_and_a_dead_holder_is_taken_over() {
    let db = TestDb::fresh().await;
    let one = PodId::new("se-cron-0").expect("a valid pod id");
    let two = PodId::new("se-cron-1").expect("a valid pod id");
...
#[tokio::test]
async fn s046_a_relay_and_a_cron_job_of_the_same_name_never_share_a_slot() {
    let db = TestDb::fresh().await;
