...
#[tokio::test]
async fn s081_engine_a_row_outside_the_visibility_is_filtered_from_the_snapshot() {
    ...
    let engine = spy_engine(
        &db,
        fabric,
        engine_config("se_s081_filter", "pod-s29a"),
...
#[tokio::test]
async fn s081_the_session_window_membership_is_derived_from_the_visibility_declaration() {
    let db = TestDb::fresh().await;
    let pool = db.app_pool().clone();
...
#[tokio::test]
async fn s081_engine_losing_the_membership_removes_the_row_from_the_live_session() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
    ...
    let engine = spy_engine(
        &db,
        fabric,
        engine_config("se_s081_revoke", "pod-s29b"),
