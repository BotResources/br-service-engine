...
#[tokio::test]
async fn s28_the_boot_full_read_projects_a_prefilled_bucket_before_it_reports_ready() {
...
#[tokio::test]
async fn s28_a_kv_put_lands_in_known_users_and_reaches_a_live_session() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
...
#[tokio::test]
async fn s28_a_kv_retract_removes_the_known_row_and_the_session_view() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
...
#[tokio::test]
async fn s28_an_empty_consumed_prefix_holds_readiness_down_and_keeps_known_star() {
    let db = TestDb::fresh().await;
    let nats = TestNats::spawn().await;
    nats.provision().await;
    let fabric = nats.nats().await;
