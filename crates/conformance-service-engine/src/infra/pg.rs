...
use roles::{ensure_app_role, grant_app_access};
...
impl TestDb {
    pub async fn fresh() -> Self {
        ...
        for role in [&app_role, &bypass_role] {
            grant_app_access(&owner, role).await;
...
async fn pool(url: &str) -> PgPool {
    service_engine::validate_database_tls(url).expect("the connection url carries a TLS posture");
    PgPoolOptions::new()
